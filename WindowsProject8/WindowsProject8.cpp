...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        SetConsoleTitleW(L"Правя граница");
        if (LOWORD(lParam) <= rect.left + 10)
            SetWindowText(hWnd, L"Левый край");
        else if (LOWORD(lParam) >= rect.right - 10) {
            SetWindowText(hWnd, L"Правый край");
        }
        else {
            SetWindowText(hWnd, L"Центр");
        }
